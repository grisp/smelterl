...
-export([
    ...
    generate_verbose_and_debug_emit_progress/1,
...
all() ->
    [
        ...
        generate_verbose_and_debug_emit_progress,
...
generate_verbose_and_debug_emit_progress(_Config) ->
    PlanPath = write_sample_plan_file(),
    OutputDir = make_temp_dir("smelterl-generate-progress"),
    ExternalDesc = filename:join(OutputDir, "main.external.desc"),
    {VerboseStatus, VerboseOutput} = run_main([
        "generate",
        "--plan", PlanPath,
        "--output-external-desc", ExternalDesc,
        "--verbose"
    ]),
    assert_equal(0, VerboseStatus),
    assert_contains(VerboseOutput, <<"generate: loading build plan">>),
    assert_contains(VerboseOutput, <<"generate: rendering target 'main'">>),
    {DebugStatus, DebugOutput} = run_main([
        "generate",
        "--plan", PlanPath,
        "--output-external-desc", ExternalDesc,
        "--debug"
    ]),
    assert_equal(0, DebugStatus),
    assert_contains(DebugOutput, <<"generate: target topology has">>),
    assert_contains(DebugOutput, <<"generate: writing external.desc">>).
