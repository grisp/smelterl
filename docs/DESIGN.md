...
| `--output-plan-env` | Bash-friendly target summary (`plan` command, optional): scalar plan metadata plus bash arrays/associative arrays for ordered target loops and normalized `extra_config`. |
...
### A.0 Example build_plan.env

**Generated by:** [§5.7.5 smelterl_cmd_plan](#575-smelterl_cmd_plan).

Shell file intended to be sourced from bash 4+. It exposes:

- scalar assignments for plan-level metadata (`ALLOY_PLAN_PRODUCT`,
  `ALLOY_PLAN_MAIN_TARGET`),
- indexed arrays for deterministic loop order
  (`ALLOY_PLAN_AUXILIARY_IDS`, `ALLOY_PLAN_TARGET_IDS`),
- associative arrays for per-target loop metadata
  (`ALLOY_PLAN_TARGET_KIND`, `ALLOY_PLAN_TARGET_ROOT`),
- one associative array carrying the normalized root-level
  `extra_config` payload (`ALLOY_PLAN_EXTRA_CONFIG`).

```bash
# Generated by smelterl - do not edit
# Source from bash for plan loop metadata.

ALLOY_PLAN_PRODUCT='acme_app'
ALLOY_PLAN_MAIN_TARGET='main'

ALLOY_PLAN_AUXILIARY_IDS=('encrypted_initramfs' 'recovery')
ALLOY_PLAN_TARGET_IDS=('encrypted_initramfs' 'recovery' 'main')

declare -A ALLOY_PLAN_TARGET_KIND=(
  ['encrypted_initramfs']='auxiliary'
  ['recovery']='auxiliary'
  ['main']='main'
)

declare -A ALLOY_PLAN_TARGET_ROOT=(
  ['encrypted_initramfs']='auxiliary_initramfs'
  ['recovery']='auxiliary_recovery'
  ['main']='acme_app'
)

declare -A ALLOY_PLAN_EXTRA_CONFIG=(
